...
{
	public MIForgeHammerMaterialRecipeContext(MaterialRegistry registry, Material material, HolderLookup.Provider registries, RecipeOutput recipes)
	{
	...
	public MIForgeHammerMaterialRecipeContext hammer(MaterialPart input, int inputCount, MaterialPart output, int outputCount, int cost)
	{
		if(this.has(input, output))
		{
			...
			new MIForgeHammerRecipeBuilder(registries)
