...
public abstract class AbstractModularCrafterComponent<R> implements MachineComponent.ServerOnly, ModularCrafterAccess<R>
{
	...
	protected abstract Identifier getRecipeId(R recipe);
	
	protected abstract R getRecipeById(Identifier recipeId);
	...
	{
		output.putLong("usedEnergy", usedEnergy);
		output.putLong("recipeEnergy", recipeEnergy);
		output.putLong("recipeMaxEu", recipeMaxEu);
		if(this.hasActiveRecipe())
		{
			output.putString("activeRecipe", this.getRecipeId(activeRecipe).toString());
		}
		else if(delayedActiveRecipe != null)
		{
			output.putString("activeRecipe", delayedActiveRecipe.toString());
		}
		output.putInt("efficiencyTicks", efficiencyTicks);
		output.putInt("maxEfficiencyTicks", maxEfficiencyTicks);
	}
	
	@Override
	public void readNbt(ValueInput input, boolean isUpgradingMachine)
	{
		usedEnergy = input.getIntOr("usedEnergy", 0);
		recipeEnergy = input.getIntOr("recipeEnergy", 0);
		recipeMaxEu = input.getIntOr("recipeMaxEu", 0);
		delayedActiveRecipe = input.getString("activeRecipe").map(Identifier::parse).orElse(null);
		...
		efficiencyTicks = input.getIntOr("efficiencyTicks", 0);
		maxEfficiencyTicks = input.getIntOr("maxEfficiencyTicks", 0);
