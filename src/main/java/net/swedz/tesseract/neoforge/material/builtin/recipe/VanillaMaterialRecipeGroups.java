...
public interface VanillaMaterialRecipeGroups
{
	MaterialRecipeGroup<VanillaMaterialRecipeContext> STANDARD = MaterialRecipeGroup.create(VanillaMaterialRecipeContext::new)
			.add("nugget_to_ingot", (context) -> context.compacting(NUGGET, INGOT, true))
			.add("main_to_block", (context) -> context.compacting(context.mainPart(), BLOCK, true))
			.add("raw_metal_to_block", (context) -> context.compacting(RAW_METAL, RAW_METAL_BLOCK, true));
