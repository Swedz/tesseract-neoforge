...
{
	public VanillaMaterialRecipeContext(MaterialRegistry registry, Material material, HolderLookup.Provider registries, RecipeOutput recipes)
	{
		super(registry, material, registries, recipes);
	}
	
	public <B extends ShapelessRecipeBuilder> VanillaMaterialRecipeContext shapeless(Function<HolderLookup.Provider, B> start, MaterialPart input, int inputCount, MaterialPart output, int outputCount, Consumer<B> builder)
	{
		if(this.has(input, output))
		{
			var inputItem = material.get(input).itemReference();
			var recipe = start.apply(registries);
			for(int __ = 0; __ < inputCount; __++)
			{
				recipe.with(RecipeHelper.ingredient(registries, inputItem));
			}
			if(builder != null)
			{
	...
	public VanillaMaterialRecipeContext shapeless(MaterialPart input, int inputCount, MaterialPart output, int outputCount, boolean inverse)
	{
		if(this.has(input, output))
		{
			...
			var recipe = new ShapelessRecipeBuilder(registries);
	...
	public <B extends ShapedRecipeBuilder> VanillaMaterialRecipeContext shaped(Function<HolderLookup.Provider, B> start, MaterialPart output, int outputCount, Consumer<ShapedRecipeMap> keyMapAction, Consumer<B> builder, String... pattern)
	{
	...
	public <B extends ShapedRecipeBuilder> VanillaMaterialRecipeContext shaped(Function<HolderLookup.Provider, B> start, MaterialPart output, int outputCount, Consumer<ShapedRecipeMap> keyMapAction, String... pattern)
	{
	...
	public VanillaMaterialRecipeContext smelting(MaterialPart input, MaterialPart output, boolean blasting, float experience)
	{
		if(this.has(input, output))
		{
			new SmeltingRecipeBuilder(registries)
