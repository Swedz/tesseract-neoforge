...
public abstract class MaterialRegistry
{
	...
	public final boolean includes(Material material, MaterialPart part)
	{
		...
		Identifier itemId = BuiltInRegistries.ITEM.getKey(material.get(part).asItem());
		return this.modId().equals(itemId.getNamespace());
	}
	
	public final void createRecipesFor(Material material, MaterialRecipeGroup recipeGroup, HolderLookup.Provider registries, RecipeOutput recipes)
	{
		recipeGroup.create(this, material, registries, recipes);
	}
	
	public final void createRecipesFor(Material material, Function<MaterialRecipeGroup, Optional<MaterialRecipeGroup>> recipeGroupFilter, HolderLookup.Provider registries, RecipeOutput recipes)
	{
		for(MaterialRecipeGroup recipeGroup : material.recipeGroups())
		{
			...
			targetRecipeGroup.ifPresent((group) -> createRecipesFor(material, group, registries, recipes));
		}
	}
	
	public final void createRecipesFor(Material material, HolderLookup.Provider registries, RecipeOutput recipes)
	{
		createRecipesFor(material, Optional::of, registries, recipes);
	}
}
