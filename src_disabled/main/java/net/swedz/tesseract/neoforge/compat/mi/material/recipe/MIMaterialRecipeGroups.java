...
public interface MIMaterialRecipeGroups
{
	...
	MaterialRecipeGroup<MIMachineMaterialRecipeContext> STANDARD_MACHINES = MaterialRecipeGroup.create(MIMachineMaterialRecipeContext::new)
			...
			.add("blade", (c) -> c.machine(PACKER, BLADE, 4, (b) -> b.partIn(CURVED_PLATE, 2).partIn(ROD, 1)))
			.add("coil", (c) -> c.machine(ASSEMBLER, CABLE, 8, COIL, 1))
			.add("large_plate", (c) -> c.machine(PACKER, PLATE, 4, LARGE_PLATE, 1))
			.add("cable_rubber", (c) -> c.machine(PACKER, CABLE, 3, (b) -> b.itemIn(MIItem.RUBBER_SHEET, 6).partIn(WIRE, 3)))
			.add("barrel", (c) -> c.machine(ASSEMBLER, BARREL, 1, (b) -> b.partIn(PLATE, 8).itemIn(Tags.Items.BARRELS_WOODEN, 1)))
			.add("tank", (c) -> c.machine(ASSEMBLER, TANK, 1, (b) -> b.partIn(PLATE, 8).itemIn(Tags.Items.GLASS_BLOCKS, 1)))
			...
			.add("crushed_dust_to_dust", (c) -> c.machine("crushed_dust", MACERATOR, DUST, 1, (b) -> b.partIn(CRUSHED_DUST, 1).addPartOutput(DUST, 1, 0.5f)))
			.add("raw_metal_to_dust", (c) -> c.machine("raw_metal", MACERATOR, DUST, 1, (b) -> b.partIn(RAW_METAL, 1).addPartOutput(DUST, 1, 0.5f)))
			...
			.add("main_to_block", (c) -> c.machine(PACKER, BLOCK, 1, (b) -> b.partIn(c.mainPart(), 9).itemIn(MIItem.PACKER_BLOCK_TEMPLATE, 1, 0f)))
			.add("block_to_main", (c) -> c.machine(UNPACKER, c.mainPart(), 9, (b) -> b.partIn(BLOCK, 1).itemIn(MIItem.PACKER_BLOCK_TEMPLATE, 1, 0f)))
			.add("ingot_to_double_ingot", (c) -> c.machine(PACKER, DOUBLE_INGOT, 1, (b) -> b.partIn(INGOT, 2).itemIn(MIItem.PACKER_DOUBLE_INGOT_TEMPLATE, 1, 0f)))
			
			.add("fuel_rod_double", (c) -> c.machine(PACKER, FUEL_ROD_DOUBLE, 1, (b) -> b.partIn(FUEL_ROD, 2).itemIn("#c:plates/nuclear_alloy", 1)))
			.add("fuel_rod_quad", (c) -> c.machine(PACKER, FUEL_ROD_QUAD, 1, (b) -> b.partIn(FUEL_ROD_DOUBLE, 2).itemIn("#c:plates/nuclear_alloy", 2)))
			...
			.add("rotor", (c) -> c.machine(ASSEMBLER, ROTOR, 1, (b) -> b.partIn(BLADE, 4).partIn(RING, 1).fluidIn(MIFluids.SOLDERING_ALLOY, 100)))
			.add("gear", (c) -> c.machine(ASSEMBLER, GEAR, 2, (b) -> b.partIn(PLATE, 4).partIn(RING, 1).fluidIn(MIFluids.SOLDERING_ALLOY, 100)))
			.add("drill_head", (c) -> c.machine(ASSEMBLER, DRILL_HEAD, 1, (b) -> b.partIn(PLATE, 1).partIn(CURVED_PLATE, 2).partIn(ROD, 1).partIn(GEAR, 2).fluidIn(MIFluids.SOLDERING_ALLOY, 75)))
			.add("cable_synthetic_rubber", (c) -> c.machine("cable_synthetic_rubber", ASSEMBLER, CABLE, 3, (b) -> b.partIn(WIRE, 3).fluidIn(MIFluids.SYNTHETIC_RUBBER, 30)))
			.add("cable_styrene_rubber", (c) -> c.machine("cable_styrene_rubber", ASSEMBLER, CABLE, 3, (b) -> b.partIn(WIRE, 3).fluidIn(MIFluids.STYRENE_BUTADIENE_RUBBER, 6)))
			
			.add("fuel_rod", (c) -> c.machine(ASSEMBLER, 16, 200, FUEL_ROD, 1, (b) -> b.itemIn("modern_industrialization:blastproof_alloy_curved_plate", 2).itemIn(MIItem.LARGE_MOTOR, 1).itemIn(MIItem.ROBOT_ARM, 2).partIn(ROD, 18).fluidIn(MIFluids.SOLDERING_ALLOY, 500).fluidIn(MIFluids.HELIUM, 100)))
			
			.add("rod_to_magnetic", (c) -> c.machine(POLARIZER, 8, 200, ROD_MAGNETIC, 1, (b) -> b.partIn(ROD, 1)))
			.add("wire_to_magnetic", (c) -> c.machine(POLARIZER, 8, 200, WIRE_MAGNETIC, 1, (b) -> b.partIn(WIRE, 1)))
			
			.add("vacuum_freezer_cool_ingot", (c) -> c.machine("hot_ingot", VACUUM_FREEZER, 32, 250, INGOT, 1, (b) -> b.partIn(HOT_INGOT, 1)))
			.add("heat_exchanger_cool_ingot", (c) -> c.machine("hot_ingot", HEAT_EXCHANGER, 8, 10, INGOT, 1, (b) -> b.partIn(HOT_INGOT, 1).fluidIn(MIFluids.CRYOFLUID, 100).fluidOut(MIFluids.ARGON, 65).fluidOut(MIFluids.HELIUM, 25)));
	...
	MaterialRecipeGroup<VanillaMaterialRecipeContext> STANDARD_MACHINE_CASING = MaterialRecipeGroup.create(MIMachineMaterialRecipeContext::new)
			.add("machine_casing", (c) -> c.machine("machine_casing", ASSEMBLER, 8, 10 * 20, MACHINE_CASING, 1, (b) -> b.partIn(PLATE, 8).partIn(GEAR, 1)))
			.add("machine_casing_pipe", (c) -> c.machine("machine_casing_pipe", ASSEMBLER, 8, 10 * 20, MACHINE_CASING_PIPE, 2, (b) -> b.partIn(MACHINE_CASING, 1).partIn(CURVED_PLATE, 6)))
