...
{
	protected final Identifier machineId;
	
	private final Comparator<T> tierSort;
	
	protected List<T>                  tiers        = List.of();
	protected Map<Identifier, T> tiersByBlock = Collections.unmodifiableMap(Maps.newHashMap());
	
	private ShapeTemplate[] shapeTemplates = new ShapeTemplate[0];
	
	public MultiblockTieredShapes(Identifier machineId, Comparator<T> tierSort)
	{
	...
	public final Map<Identifier, T> tiersByBlock()
	{
		return tiersByBlock;
	}
